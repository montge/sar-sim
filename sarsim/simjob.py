...
from sarsim import sardata, simscene
...
from . import profiling, simstate
...
def run_sim(
    ...
    if use_fmcw:
        ...
        if use_loaded_data:
            assert loaded_data is not None
            progress_callback(0.05, "Using loaded FMCW Data")
            fmcw_lines = loaded_data.fmcw_lines
        else:
            progress_callback(0.05, "FMCW Simulation")

            print(
                f"FMCW: {state.fmcw_start_frequency * 1e-9:.1f} GHz to {state.fmcw_stop_frequency * 1e-9:.1f} GHz "
                f"yields {fmcw_bw * 1e-9:.1f} GHz Bandwidth of {'up' if fmcw_up else 'down'}-Ramp."
            )

            print(
                f"FMCW: {state.fmcw_ramp_duration * 1e3:.1f} ms Ramp-Time at {state.fmcw_adc_frequency * 1e-3:.1f} kHz "
                f"yields {fmcw_samples:d} samples."
            )

            timestamper.tic("FMCW Simulation")

            fmcw_lines = _fmcw_sim(
                flight_path,
                fmcw_samples,
                scene,
                state.signal_speed,
                state.fmcw_start_frequency,
                fmcw_slope,
                fmcw_t,
                state.azimuth_3db_angle_deg,
            )
    ...
    if use_fmcw:
        ...
        progress_callback(0.25, "Range Compression")
    ...
    ## Azimuth Compression
    progress_callback(0.5, "Azimuth Compression")
    ...
    image_x, image_y, image, r_vector = _azimuth_compression(
        state, ac_use_cuda, flight_path, rc_lines, use_fmcw=use_fmcw, progress_callback=ac_progress_cb
    )

    ## Autofocus
    if state.enable_autofocus:
        progress_callback(0.75, "Autofocus")
        af_progress_cb = lambda x: progress_callback(x / 4 + 0.75, "Autofocus")
        timestamper.tic("Autofocus")
        af_image, optimal_phases = _autofocus_pafo(
            state,
            af_progress_cb,
            rc_lines,
            image,
            flight_path,
            state.autofocus_rounds,
            state.autofocus_samples,
            state.autofocus_iterations,
            use_fmcw=use_fmcw,
        )
    else:
    ...
    return SimResult(
        raw=simstate.SimImage(
            np.array(fmcw_lines), azimuth_x[0], 0, azimuth_x[1] - azimuth_x[0], 1 / state.fmcw_adc_frequency
        ),
        rc=simstate.SimImage(
            np.array(rc_lines), azimuth_x[0], r_vector[0], azimuth_x[1] - azimuth_x[0], r_vector[1] - r_vector[0]
        ),
        ac=simstate.SimImage(np.array(image), image_x[0], image_y[0], image_x[1] - image_x[0], image_y[1] - image_y[0]),
        af=simstate.SimImage(
            np.array(af_image), image_x[0], image_y[0], image_x[1] - image_x[0], image_y[1] - image_y[0]
        ),
...
if CUDA_NUMBA_AVAILABLE:
    ...
    def _ac_kernel(
        flight_path_array, image, rc_lines, image_x, image_y, PC1, PC2, r0, r_scale, r_idx_max_sub1, beamlimit, ac_wnd
    ):
...
def _azimuth_compression(
    ...
    ac_wnd = state.azimuth_compression_window.factory(
        len(flight_path), False, state.azimuth_compression_window_parameter
    )
            ...
            for ix in range(len(image_x)):
                ...
                for iy in range(len(image_y)):
                    ...
                    if sample_int < 0 or sample_int > r_idx_max_sub1 or abs(phi_a) > beamlimit:
                        ...
                        sample_int = int(sample_int)
                        sample = (
                            rc_lines[ia][sample_int] * (1 - sample_frac) + rc_lines[ia][sample_int + 1] * sample_frac
                        )
...
def _range_compression(state: simstate.SarSimParameterState, fmcw_lines: list) -> np.ndarray:
    ...
    if rc_used_bandwidth >= 0.99999999:
        wnd = state.range_compression_window.factory(
            len(fmcw_lines[0]), False, state.range_compression_window_parameter
        )
    else:
    ...
    print(
        f"Range-FFT: Have {len(wnd)} samples, minimum oversampling: {state.range_compression_fft_min_oversample:.3f}x"
    )
    ...
    rc_lines = [  # note: yes, this could be done in one numpy call, but that is actually not much faster
        np.fft.rfft(fmcw_line * wnd, n=nfft) for fmcw_line in fmcw_lines
    ]
...
def _distort_path(flight_path: np.ndarray, state: simstate.SarSimParameterState) -> np.ndarray:
    ...
    num_positions = int(
        state.distortion_sample_frequency * (state.azimuth_stop_position - state.azimuth_start_position)
    )
...
def _make_flight_path(state: simstate.SarSimParameterState) -> np.ndarray:
    ...
    path = np.array([[x, -state.flight_distance_to_scene_center, state.flight_height] for x in azimuth_x])
    # add wiggles
    unscaled_offsets = np.sin(
        np.stack((azimuth_x, azimuth_x, azimuth_x)).T
        * np.array(
            [
                state.flight_wiggle_frequency_azimuth,
                state.flight_wiggle_frequency_range,
                state.flight_wiggle_frequency_height,
            ]
        )
    )
    scaled_offsets = unscaled_offsets * np.array(
        [
            state.flight_wiggle_amplitude_azimuth,
            state.flight_wiggle_amplitude_range,
            state.flight_wiggle_amplitude_height,
        ]
    )
...
def _autofocus_pafo(
    ...
    for round in range(rounds):
        ...
        for az_index in range(len(rc_lines)):
            ...
            _, _, single_pulse, _ = _azimuth_compression(
                state,
                CUDA_NUMBA_AVAILABLE,
                flight_path[[az_index]],
                rc_lines[[az_index]],
            ...
            for iteration in range(iterations):
                ...
                if CUDA_NUMBA_AVAILABLE:
                    ...
                    for idx, factor in enumerate(candidate_factors):
                        _af_pafo_cost_function_kernel[gridsize, blocksize](
                            ac_image_gpu, single_pulse, factor, cost_function_out_gpu
                        )  # type: ignore
                        metric_sums[idx] = -_af_pafo_reduce(cost_function_out_gpu)  # type: ignore

                else:
                    metric_sums = -np.sum(
                        np.abs(ac_image + single_pulse * candidate_factors[:, np.newaxis]) ** 4, axis=1
                    )
                    # if az_index == 1:
            ...
            if max(interpol_values) - min(interpol_values) > 1e-4:
                optimal_phase = (
                    ...
                    + sample_spacing / 2
                    - sample_spacing
                    * (interpol_values[2] - interpol_values[1])
                    / (interpol_values[0] + interpol_values[2] - 2 * interpol_values[1])
                )
