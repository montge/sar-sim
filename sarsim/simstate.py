    ...
    def stringify(self, val: Any):
        ...
        if self.choices is not None:  # enum-style type
            for k, v in self.choices.items():
                ...
                raise ValueError("Value is not a valid choise for this type.")
        else:
            return str(val)
...
SAR_SIM_PARAMETERS = (
    ...
    SimParameter(
        _WINDOW, "azimuth_compression_window", "ac_wnd", SUPPORTED_WINDOWS["Rect"], category="Azimuth Compression"
    ),
    SimParameter(
        _WINDOW_PARAM, "azimuth_compression_window_parameter", "ac_wnd_param", 0, category="Azimuth Compression"
    ),
    ...
    SimParameter(
        _BOOL,
        "inverted_phase_correction",
        "invert_phase_corr",
        False,
        category="General",
        info="Only for compatibility, do not use.",
    ),
    ...
    SimParameter(
        SimParameterType(float, unit="x", min=1, max=32),
        "range_compression_fft_min_oversample",
        "rc_fft_os",
        16,
        category="Range Compression",
    ),
    SimParameter(
        _WINDOW, "range_compression_window", "rc_wnd", SUPPORTED_WINDOWS["Rect"], category="Range Compression"
    ),
...
def write_simstate_stub_file() -> None:
    ...
    with open(stub_file_name, "w", encoding="utf-8") as f:
        with open(__file__, encoding="utf-8") as me:
            f.write('"""\n')
                ...
                if line.strip() == "# <dynamic_properties>":
                    ...
                    for parameter in SAR_SIM_PARAMETERS:
                        ...
                        if parameter.type.choices is not None:
                            ...
                            f.write(
                                f"{indent}self.{parameter.name}: {parameter.type.type.__name__} = {parameter.default}\n"
                            )
                        f.write(f'{indent}"""\n')
