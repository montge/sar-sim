...
class SarData:
    ...
    def _load_cfg_values(self):
        ...
        sim.fmcw_adc_frequency = cfg["params"].getfloat(
            "adc_frequency", fallback=1.0e6
        )  # Default implied from Sensor type
        ...
        if not is_rg_comp:
            ...
            img_offset_z = cfg["params"].getfloat(
                "gbp_image_z", fallback=0
            )  # use 0 as fallback for z height for rg compressed, this makes more sense
        ...
        sim.image_count_x = round(
            sim.image_count_y * (image_region[2] - image_region[0]) / (image_region[3] - image_region[1])
        )
    ...
    def import_from_directory(cls, directory: str) -> "SarData":
        ...
        if bin_file_name is None:
            bin_candidates = [
                x for x in os.listdir(directory) if x.endswith(".bin") and x != "fmcw.bin" and x != "range_comp.bin"
            ]
        ...
        if not sd.has_range_compressed_data:
            ...
            sd.fmcw_lines = cls.load_fmcw_binary(
                os.path.join(directory, bin_file_name), sd.sim_state.azimuth_count, samples_per_rangeline
            )
        else:
            sd.rg_comp_data = cls.load_range_comp_binary(
                os.path.join(directory, bin_file_name), sd.sim_state.azimuth_count
            )
        return sd
